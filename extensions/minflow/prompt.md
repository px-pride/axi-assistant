...
- **When a card is dropped or cancelled.** Delete it with `minflow card delete` — do NOT mark it complete. Completed means the work was done and verified. Cancelled work is the opposite. If the cancellation itself is worth tracking (e.g. a deliberate decision to drop a feature), add a new already-completed card documenting the decision, rather than repurposing the original card.
