...
When this skill is used with `decision-tree`, treat it as an enhancement layer. `decision-tree` owns intensity, gating, user questions, and decision logs; this skill owns Claude-assisted critique, option generation, and candidate decision-point discovery.
...
8. Codex presents the revised options to the user. Include both recommendations explicitly for every option, except when strict `decision-tree` mode suppresses or defers visible recommendations:
...
When this skill is active with `decision-tree --strict`, strict mode controls the user-facing recommendation display:

- Present the options neutrally first, with assumptions, reversibility, failure modes, and steelman/strawman analysis as required by `decision-tree`.
...
When the user asks Claude to help find decision points, or when this skill is active alongside `decision-tree`, use Claude to draft a candidate backlog before the first user-facing fork and refresh it periodically. Good refresh triggers:
