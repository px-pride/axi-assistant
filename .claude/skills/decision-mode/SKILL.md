...
- If `decision-council` is also active, decision-mode still owns intensity, gating, user questions, and decision logs; the council skill enriches candidate discovery and option critique.
- If `decision-council` is active in strict mode, strict mode controls the recommendation display: neutral options first, no unified recommendation, and no visible model leanings unless the user explicitly asks for them or opts into seeing them for the session.
...
When strict mode and `decision-council` are both active:
...
- **Exiting**: Only when the user explicitly says "exit decision mode". Never exit implicitly.
